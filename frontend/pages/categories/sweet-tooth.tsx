...
import { Star, Plus, Clock, Truck, Candy, ArrowLeft } from 'lucide-react';
...
const SweetToothPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        console.log('Fetching products for Sweet Tooth category...');
        const response = await api.get('/products?category=' + encodeURIComponent('Sweet Tooth'));
        
        console.log('Sweet Tooth API response:', response.data);
        ...
        if (response.data.success) {
          ...
          console.log('Transformed Sweet Tooth products:', transformedProducts);
          setAllProducts(transformedProducts);
        }
      } catch (error) {
        console.error('Error fetching Sweet Tooth products:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Head>
        <title>Sweet Tooth - QuickSnack</title>
        <meta name="description" content="Delicious sweets and desserts for your sweet cravings" />
      </Head>

      <Layout>
        <div className="min-h-screen bg-gradient-to-br from-pink-50 to-purple-50 pt-20 pb-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <div className="flex items-center justify-center mb-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-gradient-to-br from-pink-500 to-purple-500 rounded-2xl text-white shadow-lg">
                    <Candy className="w-8 h-8" />
                  </div>
                  <div>
                    <h1 className="text-4xl font-bold text-slate-blue-gray">Sweet Tooth</h1>
                    <p className="text-slate-blue-gray/70 mt-2">
                      Satisfy your sweet cravings with delicious treats
                    </p>
                  </div>
                </div>
              </div>
            ...
            {loading ? (
              <div className="text-center py-16">
                ...
                <p className="text-slate-blue-gray/70 mt-4">Loading sweet treats...</p>
              </div>
              ...
              >
                <Candy className="w-16 h-16 text-slate-blue-gray/30 mx-auto mb-4" />
                <h3 className="text-2xl font-semibold text-slate-blue-gray mb-2">No Products Yet</h3>
                <p className="text-slate-blue-gray/70 mb-6">
                  We're working on adding delicious sweets and desserts to this category. Check back soon!
                </p>
                <button
                  onClick={() => router.push('/products')}
                  className="px-6 py-3 bg-gradient-to-r from-pink-500 to-purple-500 text-white font-semibold rounded-xl hover:shadow-lg transition-all duration-300 hover:scale-105"
                >
                  Browse All Products
                </button>
              </motion.div>
              ...
              >
                {allProducts.map((product: Product, index: number) => (
                  ...
                  >
                    ...
                    {/* Product Info */}
                    <div className="text-center mb-4">
                      <h3 className="font-semibold text-slate-blue-gray mb-2 group-hover:text-pink-500 transition-colors">
                        {product.name}
                      </h3>
