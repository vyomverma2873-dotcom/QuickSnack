...
import { Star, Plus, Clock, Truck, Leaf, ArrowLeft } from 'lucide-react';
...
const PaanCornerPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        console.log('Fetching products for Paan Corner category...');
        const response = await api.get('/products?category=' + encodeURIComponent('Paan Corner'));
        
        console.log('Paan Corner API response:', response.data);
        ...
        if (response.data.success) {
          ...
          console.log('Transformed Paan Corner products:', transformedProducts);
          setAllProducts(transformedProducts);
        }
      } catch (error) {
        console.error('Error fetching Paan Corner products:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <Head>
        <title>Paan Corner - QuickSnack</title>
        <meta name="description" content="Traditional paan and mouth fresheners" />
      </Head>

      <Layout>
        <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 pt-20 pb-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <div className="flex items-center justify-center mb-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-4xl font-bold text-slate-blue-gray">Paan Corner</h1>
                    <p className="text-slate-blue-gray/70 mt-2">
                      Traditional paan and mouth fresheners
                    </p>
                  </div>
                </div>
              </div>
            ...
            {loading ? (
              <div className="text-center py-16">
                ...
                <p className="text-slate-blue-gray/70 mt-4">Loading paan products...</p>
              </div>
              ...
              >
                <Leaf className="w-16 h-16 text-slate-blue-gray/30 mx-auto mb-4" />
                <h3 className="text-2xl font-semibold text-slate-blue-gray mb-2">No Products Yet</h3>
                <p className="text-slate-blue-gray/70 mb-6">
                  We're working on adding traditional paan and mouth fresheners to this category. Check back soon!
                </p>
                <button
                  onClick={() => router.push('/products')}
                  className="px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold rounded-xl hover:shadow-lg transition-all duration-300 hover:scale-105"
                >
                  Browse All Products
                </button>
              </motion.div>
              ...
              >
                {allProducts.map((product: Product, index: number) => (
                  ...
                  >
                    ...
                    {/* Product Info */}
                    <div className="text-center mb-4">
                      <h3 className="font-semibold text-slate-blue-gray mb-2 group-hover:text-green-500 transition-colors">
                        {product.name}
                      </h3>
                      ...
                      <div className="flex items-center justify-center gap-4 text-xs text-slate-blue-gray/70">
                        <div className="flex items-center gap-1">
                          ...
                          <span>{product.deliveryTime || '20 mins'}</span>
                        </div>
                        <div className="flex items-center gap-1">
